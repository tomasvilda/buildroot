...
1. Boot from TF/SD card with ramdisk
  ...
  - Insert the TF/SD card to slot, and set boot mode to mmc0
  - Power off and power on to boot from TF/SD card with ramdisk
...
2. Boot from TF/SD card with EXT4 partition on TF/SD card
  ...
  - Power off and power on to boot from TF/SD card with ext4 partition on TF/SD card.
...
3. Boot from Ethernet with NFS
  ...
  - setenv rootpath /home/user/export/rootfs ( exported nfs directory with /etc/exports file on nfs server)
  - run netboot
...
5. Boot from Nand
  ...
  - Insert the TF/SD card to slot, and set boot mode to mmc0
  - Power off and power on to boot from TF/SD card, press 'space' to interrupt u-boot and enter the u-boot console
  - Execute 'run updatesys' in u-boot console to write MLO, u-boot.img, zImage, myd_y335x.dtb, rootfs.ubi to nand
  - Set boot mode to nand and power on
